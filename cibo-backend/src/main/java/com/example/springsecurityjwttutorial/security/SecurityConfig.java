...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception { // Method to configure your app security
        http.csrf().disable() // Disabling csrf
                ...
                .antMatchers(HttpMethod.PUT,"/api/user/").hasAnyRole("BOOKER", "WORKER", "OWNER") // Allows only users with the "USER" role to make requests to the user routes
                .antMatchers(HttpMethod.POST, "/api/orders").hasRole("BOOKER")
                .antMatchers("/api/user/**").hasRole("OWNER")
                .antMatchers(HttpMethod.GET, "/api/products/**").permitAll()
                .antMatchers(HttpMethod.GET, "/api/restourants").permitAll()
                .antMatchers("/api/products/**").hasRole("OWNER")
                .and()
