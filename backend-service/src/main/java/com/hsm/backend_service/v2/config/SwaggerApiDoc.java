...
public class SwaggerApiDoc extends WebMvcConfigurationSupport {
    ...
    public Docket api() {
        return new Docket(DocumentationType.SWAGGER_2).directModelSubstitute(LocalDate.class, Date.class)
                .select()
                .apis(RequestHandlerSelectors.basePackage("com.hcm.backend_service.v2.controllers"))
    ...
    private ApiInfo apiInfo() {
        return new ApiInfoBuilder().title("HCM Apps Backend Service API").description("APIs Documentation")
                .termsOfServiceUrl("https://gitlab.com/hcm-apps/backend-service")
                .version("2.0").build();
    }
}
